	...
	public partial class App : Application
	{
	    ...
	    public App()
		{
			...
			if (!createdNew)
			{
				...
				Debug.WriteLine("检测到已有一个莠草FM在运行，程序将关闭");
				Shutdown(0);
				return;
			}
			...
			Debug.WriteLine("**********************************************************************");
			Debug.WriteLine("莠草FM启动时间：" + App.GetPreciseTime(DateTime.Now));
			Debug.WriteLine("**********************************************************************");
			Debug.WriteLine(string.Empty);
			...
			AppDomain.CurrentDomain.UnhandledException += new UnhandledExceptionEventHandler((sender, e) =>
			{
				...
				if (exceptionObject == null)
				{
					...
					Debug.WriteLine("**********************************************************************");
					Debug.WriteLine("莠草FM出现错误：" + App.GetPreciseTime(DateTime.Now));
					Debug.WriteLine("**********************************************************************");
					...
					try
					{
						...
						string path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), @"zjWeeds\莠草FM\error.log");
		...
		public static void DeleteSettings(bool useLock = true)
		{
			...
			else
			{
				try
				{
					var dataFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), @"zjWeeds\莠草FM");
